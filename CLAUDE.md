...
5. **Backup source** - only if the user configures a `"backup"` URL under `sources` in config.json. The old default (`ProjectIgnis/Images` on GitHub) was **deleted upstream** — do not re-add it as a default.
...
## What's New panel (README)
The README embeds two generated SVGs: `assets/whats-new.svg` (latest-release card with NEW/FIX chips per change and a commits/files/lines stats row) and `assets/release-history.svg` (feature timeline — the last 8 releases, each listing the changes it shipped). **Never edit the SVGs by hand** — both are generated by `tools/build_whats_new.py` from git history ("Bump version"/"Merge" commits are skipped). The `whats-new.yml` workflow regenerates and commits them to main on every `v*` tag push, so they update themselves with each release. Preview locally with `python3 tools/build_whats_new.py`. Because bullets come from commit subjects, keep commit messages short and user-readable; the chip is picked from the first word (Add/Support/Enable → NEW, Fix/Stop/Prevent → FIX, else MISC).

## Launcher auto-update
All three launchers keep their cached app copy up to date: on each run they ask GitHub for the latest release tag (short timeout; skipped silently when offline) and compare it to the version recorded in `binary_version.txt` next to the cached binary. On mismatch they delete the cached binary and re-download (with the usual checksum verification). The zip-bundle copies (binary shipped next to the launcher) are never auto-updated. This matters because the in-app "new version available" nag can't update the launcher's cached copy by itself.
