...
The README embeds `assets/whats-new.svg`, a styled card listing the changes in the latest release. **Never edit the SVG by hand** — it is generated by `tools/build_whats_new.py` from the commit subjects between the two most recent version tags ("Bump version"/"Merge" commits are skipped). The `whats-new.yml` workflow regenerates and commits it to main on every `v*` tag push, so it updates itself with each release. Preview locally with `python3 tools/build_whats_new.py`. Because bullets come from commit subjects, keep commit messages short and user-readable.
