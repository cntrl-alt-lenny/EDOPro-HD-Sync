...
**Release packaging requirements must be preserved.** Publish **exactly one asset per platform** — no checksum files, no standalone launchers — so the Releases page reads simply "Windows, macOS, Linux":
- `EDOPro-HD-Sync-Windows-vVERSION.zip`
- `EDOPro-HD-Sync-macOS-vVERSION.zip`
- `EDOPro-HD-Sync-Linux-vVERSION.zip`

**Never publish a bare launcher (`.command`/`.sh`) as a release asset.** A browser download strips the executable bit (mode 644), so double-clicking it fails with "you do not have appropriate access privileges" — the exact bug reported in v5.0.0. A `.zip` preserves the bit, which is why the launcher only ships inside an archive; `build.yml` has a step that fails the build if the launcher isn't executable inside the zip. Keep `.zip` (not `.7z`) for native unzip support.

Each zip contains: the launcher, a platform-specific `ReadMe.txt`, and `app/` holding the built app plus a `version.txt` (the tag, e.g. `v5.1.0`) used to seed the installed-version marker.

**Build with PyInstaller `--onedir`, never `--onefile`.** One-file binaries re-extract ~50 MB to a temp dir on *every* launch: measured **8.1 s of startup, every run**, versus **0.2 s** for onedir (same app, same machine). Since the app already ships inside a zip's `app/` folder, onedir costs the user nothing — they never look inside it.
...
## Launcher install & auto-update
The launcher **always runs the copy it manages** in the support dir's `app/` folder (`~/Library/Application Support/EDOPro-HD-Sync/app`, `~/.local/share/EDOPro-HD-Sync/app`, `%LOCALAPPDATA%\EDOPro-HD-Sync\app`), never the one in the extracted folder — so updates behave the same no matter where the user put things. First run seeds that copy from `app/` next to the launcher (instant, no download); if the bundle is missing it falls back to downloading the release zip. Each run then compares the latest GitHub tag against `binary_version.txt` and reinstalls when they differ.

**Update order is sacred: download + verify + unpack into a scratch dir FIRST, swap the app folder LAST** (via `swap_in_app_dir` / `Swap-InAppDir`, which keeps the old folder until the move succeeds and rolls back on failure) — a failed update must never break a working install (on failure they print "keeping the current version" and run the old binary; verified by test). Downloads are verified against the SHA-256 `digest` GitHub publishes for each asset in the releases API — that's why no `.sha256` assets are needed. The shell launchers parse that JSON with sed/awk (no python3 — stock Macs don't have it), the .bat passes its own path via the `HDSYNC_SELF` env var (so `$SelfDir` works from the temp-extracted PowerShell and apostrophe paths are safe), and `looks_like_edopro` mirrors `get_db_files()` exactly so the launcher and app never disagree about a folder.
